...
const Users = ({ eventId, refreshUsers }) => {
  ...
  useEffect(() => {
    // Cada vez que el tab de "Usuarios" se activa, refrescar la lista
    ...
    // Limpiar los mensajes de error y éxito cada vez que el componente es renderizado
    setErrorMessage('');
    setSuccessMessage('');
  }, [eventId, refreshUsers]); // Se refresca cuando cambia el `refreshUsers` (cuando se activa el tab)
